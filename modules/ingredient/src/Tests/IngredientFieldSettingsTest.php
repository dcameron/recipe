...
use Drupal\ingredient\Tests\IngredientTestBase;
 ...
 * @group recipe
 */
class IngredientFieldSettingsTest extends IngredientFieldTestBase {
  ...
  public function testIngredientFieldSettings() {
    ...
    $this->drupalGet('admin/structure/types/manage/test_bundle/fields/node.test_bundle.' . $field_name);
    ...
    $this->assertOptionSelected('edit-' . $field_name . '-0-unit-key', '', 'The empty unit option was selected.');
    ...
    $this->updateIngredientField($field_name, 'node', 'test_bundle', $field_settings);
    $this->drupalGet('node/add/test_bundle');
    $this->assertFieldByXPath("//option[@value='cup']", t('cup (c)'), 'Found an option from the U.S. customary unit set.');
